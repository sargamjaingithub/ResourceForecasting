...
BASE_NAME = '1_set_experiments_google_type_1_hyperparameter_search_informer'


PARAMETERS_TO_TEST = [
    {'keys': ['data', 'type', 'lag_size'], 'values': [32, 128, 512]},
    {'keys': ['model', 'type', 'train_info', 'optimizer', 'args', 'lr'], 'values': [0.1, 0.01, 0.001]},
    {'keys': ['model', 'network', 'args', 'd_model'], 'values': [32, 128], 'auxiliary_keys': ['model', 'network', 'args', 'd_ff']},
    {'keys': ['model', 'network', 'args', 'n_heads'], 'values': [2, 4]},
    {'keys': ['model', 'network', 'args', 'e_layers'], 'values': [2, 4], 'auxiliary_keys': ['model', 'network', 'args', 'd_layers']},
    {'keys': ['model', 'network', 'args', 'dropout'], 'values': [0.0, 0.5]}
]
...
def create_config_rec(base_config: dict, left_parameters_to_set: list, already_set_parameters: list):
    if len(left_parameters_to_set) == 0:
        ...
        for already_set_parameter in already_set_parameters:
            ...
            if 'auxiliary_keys' in already_set_parameter: 
                auxiliary_keys = already_set_parameter['auxiliary_keys']
                __nested_set(new_config, auxiliary_keys, value)
        ...
        if 'auxiliary_keys' in left_parameter_to_set:
            auxiliary_keys = left_parameter_to_set['auxiliary_keys']
        ...
        for value in values:
            ...
            if 'auxiliary_keys' in left_parameter_to_set:
                auxiliary_object['auxiliary_keys'] = auxiliary_keys
            auxiliary_list.append(auxiliary_object)
