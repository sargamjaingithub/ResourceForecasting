...
class DataTypeFullValuesUnbiasedIntervalsZeroExponential(DataTypeFullValuesUnbiasedIntervals):

    def __init__(self, config, *args):
        super().__init__(config, *args)
        reduction_magnitude = config['reduction_magnitude']

        self.proportion = torch.ones(len(self.intervals), dtype=torch.float)
        for index in range(1, self.proportion.shape[0]):
            self.proportion[index:] /= reduction_magnitude
