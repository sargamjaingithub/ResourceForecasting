...
class DataTypeFullValuesUnbiasedIntervalsPrioBalanced(DataTypeFullValuesUnbiasedIntervals):
    ...
    def __init__(self, config, *args):
        ...
        self.spikes_balance = config['spikes_balance']
        ...
        for time_series_id_index in range(len(time_series_ids)):
            ...
            if is_train:
                ...
                compare_multiple_lines_points_color(
                    ...
                    [
                        (
                            time_series_values_values_list[time_series_id_index][target_feature_index_values],
                            np.arange(time_series_length),
                            self.output_values_color,
                            1,
                            'values'
                        ),
                        (
                            time_series_target_values_list[time_series_id_index],
                            np.arange(time_series_length),
                    ...
                    [(time_series_target_values_list[time_series_id_index][time_position], time_position) for (time_series_id_index, time_position) in possible_time_values_spikes],
                    'feature',
                    'instant',
                    f'complete',
                ...
                '''
                checker_len_indexes = len(indexes)
                count_default = len(possible_time_values_default)
                count_spikes = len(possible_time_values_spikes)
                count_total = count_default + count_spikes
                print(f'Initial balance -> default: {count_default} {count_default / count_total * 100}; spikes: {count_spikes} {count_spikes / count_total * 100}')

                if count_spikes == 0 or count_default == 0:
                    ...
                    current_spikes_balance = count_spikes / count_total
                    actual_spikes_balance = current_spikes_balance if current_spikes_balance > self.spikes_balance else self.spikes_balance
                    count_default = int((1 - actual_spikes_balance) * maximum_per_time_series)
                    count_spikes = maximum_per_time_series - count_default
                    count_total = count_default + count_spikes

                    if count_default > len(possible_time_values_default):
                        indexes += possible_time_values_default
                        indexes += random.choices(possible_time_values_default, k=(count_default - len(possible_time_values_default)))
                    else:
                        indexes += random.sample(possible_time_values_default, count_default)
                    if count_spikes > len(possible_time_values_spikes):
                        indexes += possible_time_values_spikes
                        indexes += random.choices(possible_time_values_spikes, k=(count_spikes - len(possible_time_values_spikes)))
                    else:
                        indexes += random.sample(possible_time_values_spikes, count_spikes)
                    assert (len(indexes) == checker_len_indexes + count_total)
                print(f'Final balance -> default: {count_default} {count_default / count_total * 100}; spikes: {count_spikes} {count_spikes / count_total * 100}')
